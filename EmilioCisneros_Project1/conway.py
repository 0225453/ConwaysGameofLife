...
def update(frameNum, img, grid, N,M):
    ...
    for i in range(N):
        for j in range(M):
            #Block
            if(checkFigure(i,j,N,M,grid,block)): numBlocks +=1
            #Beehive
            if(checkFigure(i,j,N,M,grid,beehive)): numBeehive +=1
            #Loaf
            if(checkFigure(i,j,N,M,grid,loaf)): numLoaf +=1
            #Boat
            if(checkFigure(i,j,N,M,grid,boat)): numBoat +=1
            #Tub
            if(checkFigure(i,j,N,M,grid,tub)): numTub +=1
            #Blinker
            if(checkFigure(i,j,N,M,grid,blinker1)): numBlinker +=1
            if(checkFigure(i,j,N,M,grid,blinker2)): numBlinker +=1
            #Toad
            if(checkFigure(i,j,N,M,grid,toad1)): numToad +=1
            if(checkFigure(i,j,N,M,grid,toad2)): numToad +=1
            #Beacon
            if(checkFigure(i,j,N,M,grid,beacon1)): numBeacon +=1
            if(checkFigure(i,j,N,M,grid,beacon2)): numBeacon +=1
            #Glider
            if(checkFigure(i,j,N,M,grid,glider1)): numGlider +=1
            if(checkFigure(i,j,N,M,grid,glider2)): numGlider +=1
            if(checkFigure(i,j,N,M,grid,glider3)): numGlider +=1
            if(checkFigure(i,j,N,M,grid,glider4)): numGlider +=1
            #Light-weight spaceship
            if(checkFigure(i,j,N,M,grid,lws1)): numLWS +=1
            if(checkFigure(i,j,N,M,grid,lws2)): numLWS +=1
            if(checkFigure(i,j,N,M,grid,lws3)): numLWS +=1
            if(checkFigure(i,j,N,M,grid,lws4)): numLWS +=1
    ...
    if(sumTotal!= 0): f.write(" "+ str(numBlocks/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Beehive: "+ str(numBeehive) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numBeehive/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Loaf: "+ str(numLoaf) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numLoaf/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Boat: "+ str(numBoat) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numBoat/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Tub: "+ str(numTub) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numTub/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Blinker: "+ str(numBlinker) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numBlinker/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Toad: "+ str(numToad) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numToad/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Beacon: "+ str(numBeacon) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numBeacon/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("Glider: "+ str(numGlider) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numGlider/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
    f.write("LWS: "+ str(numLWS) +" | ")
    if(sumTotal!= 0): f.write(" "+ str(numLWS/sumTotal)+" % "+"\n")
    else:  f.write(" "+ str(0)+" % "+"\n")
